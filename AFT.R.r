...
for (iter in 2:mcsim){
  ...
  for (i in 1:N_grp){
    ...
    dens.long[1]<-dmvn(z[grp==i], mean.long_grp1, sig.long, log=TRUE) + sum(omega_short[grp_short==i]*log(exp(-lambda[iter-1]*t_short[grp_short==i]^r[iter-1]*exp(-r[iter-1]*eta_1))*(lambda[iter-1]*exp(-r[iter-1]*eta_1)*r[iter-1]*t_short[grp_short==i]^(r[iter-1]-1))) + (1-omega_short[grp_short==i])*(-lambda[iter-1]*t_short[grp_short==i]^r[iter-1]*exp(-r[iter-1]*eta_1))) #take log
    dens.long[2]<-dmvn(z[grp==i], mean.long_grp2, sig.long, log=TRUE) + sum(omega_short[grp_short==i]*log(exp(-lambda[iter-1]*t_short[grp_short==i]^r[iter-1]*exp(-r[iter-1]*eta_2[iter-1]))*(lambda[iter-1]*exp(-r[iter-1]*eta_2[iter-1])*r[iter-1]*t_short[grp_short==i]^(r[iter-1]-1))) + (1-omega_short[grp_short==i])*(-lambda[iter-1]*t_short[grp_short==i]^r[iter-1]*exp(-r[iter-1]*eta_2[iter-1]))) #take log
  ...
  # log density of eta_2
  logden.eta_2 <- function(eta_2, lambda, r) {
    return(sum(omega_short[cluster.ind==2]*log(exp(-lambda*t_short[cluster.ind==2]^r*exp(-r*eta_2))*(lambda*exp(-r*eta_2)*r*t_short[cluster.ind==2]^(r-1))) + (1-omega_short[cluster.ind==2])*(-lambda*t_short[cluster.ind==2]^r*exp(-r*eta_2))) + dunif(eta_2, -1, 1, log=TRUE))
  }
  # log density of lambda
  logden.lambda <- function(lambda, eta_2, r) {
    return(sum(omega_short[cluster.ind==1]*log(exp(-lambda*t_short[cluster.ind==1]^r*exp(-r*eta_1))*(lambda*exp(-r*eta_1)*r*t_short[cluster.ind==1]^(r-1))) + (1-omega_short[cluster.ind==1])*(-lambda*t_short[cluster.ind==1]^r*exp(-r*eta_1)))
           + sum(omega_short[cluster.ind==2]*log(exp(-lambda*t_short[cluster.ind==2]^r*exp(-r*eta_2))*(lambda*exp(-r*eta_2)*r*t_short[cluster.ind==2]^(r-1))) + (1-omega_short[cluster.ind==2])*(-lambda*t_short[cluster.ind==2]^r*exp(-r*eta_2))) + dgamma(lambda, shape=0.1, rate=0.1, log=TRUE))
  }
  # log density of r
  logden.r <- function(r, eta_2, lambda) {
    return(sum(omega_short[cluster.ind==1]*log(exp(-lambda*t_short[cluster.ind==1]^r*exp(-r*eta_1))*(lambda*exp(-r*eta_1)*r*t_short[cluster.ind==1]^(r-1))) + (1-omega_short[cluster.ind==1])*(-lambda*t_short[cluster.ind==1]^r*exp(-r*eta_1)))
           + sum(omega_short[cluster.ind==2]*log(exp(-lambda*t_short[cluster.ind==2]^r*exp(-r*eta_2))*(lambda*exp(-r*eta_2)*r*t_short[cluster.ind==2]^(r-1))) + (1-omega_short[cluster.ind==2])*(-lambda*t_short[cluster.ind==2]^r*exp(-r*eta_2))) + dgamma(r, shape=0.1, rate=0.1, log=TRUE))
  }
  
  # update eta_2
  eta_2.proposal = function(x){return(rnorm(1,x,0.01))}
  eta_2.proposed = eta_2.proposal(eta_2[iter-1])
  prob.eta_2 = exp(logden.eta_2(eta_2.proposed, lambda[iter-1], r[iter-1])-logden.eta_2(eta_2[iter-1], lambda[iter-1], r[iter-1]))
  if (runif(1) < prob.eta_2){
    eta_2[iter] = eta_2.proposed
  }else{
    eta_2[iter] = eta_2[iter-1]
  }
  # update lambda
  lambda.proposal = function(x){return(runif(1,max(0,x-0.1),x+0.1))}
  lambda.proposed = lambda.proposal(lambda[iter-1])
  prob.lambda = exp(logden.lambda(lambda.proposed, eta_2[iter], r[iter-1])-logden.lambda(lambda[iter-1], eta_2[iter], r[iter-1]))
  ...
  prob.r = exp(logden.r(r.proposed, eta_2[iter], lambda[iter])-logden.r(r[iter-1], eta_2[iter], lambda[iter]))
  if (runif(1) < prob.r){
    r[iter] = r.proposed
  }else{
    r[iter] = r[iter-1]
...
sum(((1/lambda/exp(-r*eta_2))^(1/r)*(log(2))^(1/r))[(nburnin+1):mcsim]>0.6)/(mcsim-nburnin)
...
if (sum(((1/lambda/exp(-r*eta_2))^(1/r)*(log(2))^(1/r))[(nburnin+1):mcsim]>0.6)/(mcsim-nburnin)<0.05){
  for (i in 1:I){if (mean(ci[(nburnin+1):mcsim,1,i])<=0.5){
    longdata <- longdata[(longdata[,1] !=i),]
  }}}
...
if (length(unique(longdata[,1]))!=0){
  ...
  print(sum(((1/lambda/exp(-r*eta_2))^(1/r)*(log(2))^(1/r))[(nburnin+1):mcsim]>0.6)/(mcsim-nburnin))
}
